...
const Notifications = ({changeLanguage}) => {
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white p-8 rounded-2xl shadow">
        ...
        {suggestions.length > 0 ? (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <p className="text-sm font-semibold text-gray-900">
                    {t(`suggestions.types.${suggest.type}`, suggest.type)}
                  </p>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${getStatusStyle(suggest.state)}`}>
                    {t(`suggestions.status.${suggest.state}`, 'Pending')}
                  </span>
                </div>
          ...
          <p className="text-gray-500">{t('suggestions.empty')}</p>
        )}
      </div>
    </div>
  </div>
